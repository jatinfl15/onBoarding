...
public struct OnboardingItemInfo {
    ...
    public init (informationImage: UIImage, title: String, description: String, pageIcon: UIImage, color: UIColor, titleColor: UIColor, descriptionColor: UIColor, titleFont: UIFont, descriptionFont: UIFont, descriptionLabelPadding: CGFloat = 0, titleLabelPadding: CGFloat = 0,  index: Int = 0) {
...
public extension PaperOnboarding {
    ...
    func currentIndex2(_ index: Int, animated: Bool) {
        if 0 ..< itemsCount ~= index {
            (delegate as? PaperOnboardingDelegate)?.onboardingWillTransitonToIndex(index)
            currentIndex = index
            CATransaction.begin()
            ...
            if var postion = pageView?.positionItemIndex(index, onView: self) {
                postion.x = (pageView?.frame.size.width)! / 2
                fillAnimationView?.fillAnimation2(backgroundColor(currentIndex), centerPosition: postion, duration: 0.5)
            }
            pageView?.currentIndex(index, animated: animated)
            contentView?.currentItem(index, animated: animated)
            CATransaction.commit()
...
//        }else {
//            currentIndex2(index, animated: true)
//        }
        (delegate as? PaperOnboardingDelegate)?.onboardingWillTransitonToIndex(index)
    }
